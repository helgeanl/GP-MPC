...
simTime = 1800                           # Simulation time in seconds
...
def integrate_system(ndstate,nastate,u,t0,tf,x0):
    ...
    # Differential states
    xd   =  SX.sym("xd",ndstate) # vector of states [c1,c2,I]
    
    # Algebraic states
    xa =    SX.sym("xa",nastate) # vector of algebraic states [mu1,mu2]
    ...
    alg = vertcat(mu1_M*(Sf-xd[0]/y1-xd[1]/y2)/(K+(Sf-xd[0]/y1-xd[1]/y2))-xa[0],\
...
if sim is True:
    ...
    time = np.linspace(0.0, simTime,simPoints)
